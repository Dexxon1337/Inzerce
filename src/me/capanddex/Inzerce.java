				...
				if (args.length == 0) {
					...
					if (!ad.equals(null)) {
						this.getServer().broadcastMessage(
								"Hrac " + ad.getAdvertiser().getDisplayName()
										+ " prodává " + ad.getCount() + " ks "
										+ ad.getMat().toString() + ". Cena: €"
										+ ad.getPrice());
						list.add(ad);
						counter++;
						return true;
					...
					for (int i = 0; i < list.size(); i++) {
						player.sendMessage("ID: " + list.get(i).getID()
								+ " Inzerent: "
								+ list.get(i).getAdvertiser().getDisplayName()
								+ " Pøedmìt: "
								+ list.get(i).getMat().toString().toLowerCase()
								+ " Poèet: " + list.get(i).getCount()
								+ " Cena: " + list.get(i).getPrice());
					}
